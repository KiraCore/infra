...
KIRA_SETUP_KIRA_ENV="$KIRA_SETUP/kira-env-v0.0.40" 
if [ ! -f "$KIRA_SETUP_KIRA_ENV" ] ; then
    ...
    CDHelper text lineswap --insert="KIRA_VALIDATORS_SUBNET=$KIRA_VALIDATORS_SUBNET" --prefix="KIRA_VALIDATORS_SUBNET=" --path=$ETC_PROFILE --append-if-found-not=True
